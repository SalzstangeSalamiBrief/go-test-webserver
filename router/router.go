...
func HandleRouting(w http.ResponseWriter, r *http.Request) {
	...
	if hasMatched == false {
		handler.ErrorHandler(w, fmt.Sprintf("Could not get resource '%v' with method '%v'", r.URL.Path, r.Method), http.StatusBadRequest)
	}
}
...
func validateHttpMethod(w http.ResponseWriter, r *http.Request) bool {
	...
	if isAccepted == false {
		handler.ErrorHandler(w, fmt.Sprintf("The method '%v' is not acceptable", r.Method), http.StatusBadRequest)
	}
